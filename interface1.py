...
class MyGui(Frame):
    def __init__(self, parent=None):
        ...
        Label(self, text='Количество примеров:').grid(row=0, column=0)
        count = Entry(self)
        count.grid(row=0, column=1)
        Label(self, text='Количество слогаемых в примере:').grid(row=1, column=0)
        sum_cont = Entry(self)
        sum_cont.grid(row=1, column=1)
        Label(self, text='Максимальная степень:').grid(row=2, column=0)
        deg = Entry(self)
        deg.grid(row=2, column=1)
        Label(self, text='Расброс коэффициентов:').grid(row=3, column=0)
        cof = Entry(self)
        cof.grid(row=3, column=1)


        Button(self, text='Add', command=(lambda: self.add(int(count.get()),int(sum_cont.get()),int(deg.get()),int(cof.get()))).grid(row=10, column=0))
    ...
    def add(count = 10, sum_count = 5, deg = 4, cof = 20):
        showinfo(title='popup', message='Готово!')
